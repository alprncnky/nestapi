...
export const UpdateRssSourceMapping = {
  name: () => StringField('RSS source name', 'Bloomberg HT', false, 3, 255),
  url: () => StringField('RSS feed URL', 'https://www.bloomberght.com/rss', false, 10, 500),
  feedType: () => EnumField(FeedTypeEnum, 'RSS feed type', FeedTypeEnum.RSS2, false),
  category: () => EnumField(SourceCategoryEnum, 'Source category', SourceCategoryEnum.COMPANY_NEWS, false),
  country: () => StringField('Country code (ISO 3166-1 alpha-2)', 'TR', false, 2, 10),
  fetchInterval: () => NumberField('Fetch interval in minutes', 60, false, 5, 1440),
  isActive: () => BooleanField('Is source active', true, false),
