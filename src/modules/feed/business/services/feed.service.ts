...
export class FeedService {
  ...
  async jobExecute(): Promise<{ saved: number; skipped: number; errors: number }> {
    ...
    this.logger.log(
      `Feed job completed. Saved: ${savedCount}, Skipped: ${skippedCount}, Errors: ${errorCount}`,
    );
  ...
  private async processFeedSource(
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch from ${rssSource.source}: ${error.message}`,
        error.stack,
      );
      errors++;
    }
